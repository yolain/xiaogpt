...
class MiAccount:
    ...
    async def _securityTokenService(self, location, nonce, ssecurity):
        ...
        async with self.session.get(location + '&clientSign=' + parse.quote(clientSign), ssl=False) as r:
    ...
    async def mi_request(self, sid, url, data, headers, relogin=True):
        if (self.token and sid in self.token) or await self.login(sid):  # Ensure login
            ...
            async with self.session.request(method, url, data=content, cookies=cookies, headers=headers, ssl=False) as r:
                status = r.status
                if status == 200:
